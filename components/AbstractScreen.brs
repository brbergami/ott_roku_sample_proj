...
sub configureScreen(config as Object)
    ...
    m.title.translation              = [m.displaySize.w * 0.45, m.displaySize.h * 0.3] ' well, seems like SG getDisplaySize reports max 720p screen on old devices and renders ugly
    m.title.font.size                = 48
    m.description.text               = config.description
    m.description.translation        = [m.displaySize.w * 0.38, m.displaySize.h * 0.4] ' I should substract here witdh and height of the Label too, but doesn't seems to update the 0 after setting the text
    m.description.font.size          = 24
    m.top.getScene().backgroundUri   = ""
    m.top.getScene().backgroundColor = config.background
